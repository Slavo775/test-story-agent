...
export async function resolveTargets({ mode, filesCsv, owner, repo, pr, baseBranch, include, exclude, }) {
    if (filesCsv) {
        return filesCsv
            .split(",")
            .map((s) => s.trim())
...
function match(files, include, exclude) {
    const inc = include.map((p) => new Minimatch(p));
    const exc = exclude.map((p) => new Minimatch(p));
    return files.filter((f) => inc.some((m) => m.match(f)) && !exc.some((m) => m.match(f)));
}
